...
| 04 | `2026-07-03-review-04-split-test-ui.md` | Split the 2,988-line `test_ui.py` into ~10 per-area modules + `tests/support.py`; zero behavior change | Done | — | Sonnet 4.6 | Low | PR branch `review-04-split-test-ui`; collected count stayed `181`; UI suite prints `181 passed, 11 subtests passed` |
| 05 | `2026-07-03-review-05-unify-render-invocation.md` | Single `CoreBackend.render(RenderRequest)` + single status formatter; remove redundant widget debounce | Done | 03, 04 | Sonnet 4.6 | Medium | PR branch `worktree-review-05-unify-render-invocation`; `to_render_kwargs` has exactly one consumer (`backend.py`); suite `185 passed` (181 + 4 new tests) |
