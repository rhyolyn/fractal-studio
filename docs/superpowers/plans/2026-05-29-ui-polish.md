...
**Architecture:** Two new focused widgets (`SectionPanel`, `ViewportWell`) replace `QGroupBox` throughout `sections.py`. Five new `ThemeSpec` tokens drive all new colours via QSS. Collapse state lives in `UiSettings`; save orchestration goes through `SettingsController` (not `MainWindow`). `ThemeController` owns the active `ThemeSpec` from the very first call, including startup. Implemented in three phases: Phase 1 (Tasks 1–4) new widgets and theme tokens; Phase 2 (Tasks 5–7) layout migration and sidebar; Phase 3 (Tasks 8–10) dialog, favorites, and visual verification.
...
| Modify | `ui/src/fractal_studio/application/controllers/settings_controller.py` | Add `save_sidebar_collapsed` method |
| Modify | `ui/src/fractal_studio/application/workflows/startup_coordinator.py` | Accept `ThemeController`; add `sidebar_collapsed` to `WindowStartupState` |
...
`_SectionHeader` is a proper `QWidget` subclass that overrides `mousePressEvent` cleanly — no inline lambda patching. The chevron is a flat `QToolButton` so it's a real interactive widget, not a label pretending to be one. `set_header_widget` appends to a dedicated slot position in the header layout — no layout introspection or index searching.
...
**Sizing contract:** `FractalViewportWidget` uses `QSizePolicy(Expanding, Fixed)` with `hasHeightForWidth() = True`. When placed inside `ViewportWell`'s `QHBoxLayout` with `AlignHCenter | AlignVCenter`, Qt honours `hasHeightForWidth` on the viewport and sizes it correctly — height follows from width at the chosen aspect ratio. The well itself has `QSizePolicy(Expanding, Expanding)` so it grows to fill the panel; leftover space around the canvas shows the checkerboard. This must be verified with a smoke test in all three aspect ratio modes (square, portrait, landscape) — see Task 10.
...
- [ ] **Step 3: Write and run a sizing contract test**

Add to `ui/tests/test_section_panel.py` (reuse the existing `app` fixture):
...
`ViewportWell` must implement `hasHeightForWidth()` returning `True` and delegate `heightForWidth(w)` to the wrapped viewport so Qt's layout engine propagates sizing correctly up the tree.

Add to `viewport_well.py`:
...
```bash
cd ui && git add src/fractal_studio/ui/widgets/viewport_well.py src/fractal_studio/ui/widgets/__init__.py tests/test_section_panel.py && git commit -m "feat: add ViewportWell with checkerboard dead space and sizing contract"
```
...
## Task 6: `UiSettings` — add `sidebar_collapsed`; route startup through `ThemeController`
...
- [ ] **Step 3: Route startup theme application through `ThemeController`**

Currently `WindowStartupCoordinator.bootstrap()` calls the module-level `apply_theme()` directly, so `ThemeController._current_theme` is `None` at startup and `ViewportWell` receives no initial theme. Fix: pass `ThemeController` into `bootstrap()`.

In `startup_coordinator.py`, update the constructor and `bootstrap`:
...
Update `WindowStartupState` to add `sidebar_collapsed`:
...
Update `bootstrap()` — call `theme_controller.apply_theme()` instead of the module-level function:
...
Remove the now-unused `from fractal_studio.theme import ThemeSpec, apply_theme` import; keep `ThemeSpec` if still referenced.

- [ ] **Step 4: Pass `theme_controller` to `WindowStartupCoordinator` in the factory**

In `main_window_factory.py`, update the `startup` construction line:

```python
startup = WindowStartupCoordinator(settings_repo, settings_service, theme_controller)
```

Note: `theme_controller` is constructed just before `startup` in `build_main_window_context` — no reordering needed.
...
```bash
cd ui && git add src/fractal_studio/state.py src/fractal_studio/application/workflows/startup_coordinator.py src/fractal_studio/main_window_factory.py && git commit -m "feat: add sidebar_collapsed to UiSettings; route startup theme through ThemeController"
```
...
Add `_on_section_collapsed` — delegates to `SettingsController`, which owns the save logic; `MainWindow` does not call the repo directly:
...
Add `save_sidebar_collapsed` to `SettingsController` (`settings_controller.py`):
...
Add the necessary imports to `settings_controller.py`:
```python
...
- [ ] **Step 8: Verify export signal connections explicitly**

The following three connections must all survive the refactor into `_build_export_section`. Verify each is present in the new code before committing:

1. `export_combo.currentIndexChanged.connect(ports.on_export_preset_changed)` — updates custom-size row visibility
2. `ports.on_export_preset_changed(export_combo.currentIndex())` — sets initial state
3. `ports.apply_aspect_ratio_mode(update_combo=False)` — syncs aspect ratio on build

Also verify that `ports.set_custom_size_boxes(custom_width_box, custom_height_box)` is called so the coordinator can read the boxes later.

Run the existing export panel tests:

```bash
cd ui && python -m pytest -m integration -k "export" -v
```
Expected: all pass
...
```bash
cd ui && git add src/fractal_studio/ui/sections/sections.py src/fractal_studio/main_window.py src/fractal_studio/application/controllers/settings_controller.py && git commit -m "feat: collapsible sidebar sections with persisted collapse state via SettingsController"
```
...
**Approach:** Additive changes to the existing dialog — do not replace the whole file. The dialog already uses a sidebar/stack structure (custom nav, not `QTabWidget`). The spec incorrectly said `QTabWidget`; the custom sidebar/stack is the right choice and already exists. Add to it.

**Files:**
- Modify: `ui/src/fractal_studio/ui/dialogs/appearance_settings_dialog.py`
- Modify: `ui/src/fractal_studio/main_window.py`

- [ ] **Step 1: Add `BackendProfile` and `backend_loaded` parameters to `AppearanceSettingsDialog.__init__`**

Change the constructor signature only — do not replace other methods:
...
- [ ] **Step 2: Extend `_build_root` to build a content stack**

Replace `_build_root` to wire up two content panels (appearance + environment) in a stack, with visibility toggled by nav buttons:
...
- [ ] **Step 3: Add "Environment" nav button to `_build_sidebar`**

In `_build_sidebar`, replace the stub nav buttons block. Keep existing appearance button; add a real Environment button and keep the others disabled:
...
- [ ] **Step 4: Add `_build_environment_content`, `_show_appearance`, `_show_environment`**

Add these three methods to the class (no existing methods are removed):
...
- [ ] **Step 5: Update `MainWindow._open_settings` to pass `backend_profile`**
...
**What's already correct and needs no change:**
- Selected row styling (`border-left: 4px solid selected_border` + `selection_bg` fill) is already implemented in `FavoriteRowStylePresenter` — do not change it.
- Timestamps: `FavoriteThumbnailRow` shows only `fav["name"]`; there is no timestamp field displayed anywhere in the current UI. The spec mentioned timestamps in error — no change needed here.
