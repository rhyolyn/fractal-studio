...
- [ ] **Step 1: Update `refresh_dynamic_widgets`**
...
class ThemeController:
    ...
    def refresh_dynamic_widgets(
        ...
        if viewport_well is not None:
            theme = getattr(QApplication.instance(), "_current_theme_spec", None)
            if theme is not None:
                viewport_well.set_theme(theme)
```

- [ ] **Step 2: Store the active theme spec on the `QApplication` instance in `apply_theme`**

In `theme.py`, update `apply_theme` to store the spec on the app so `ThemeController` can retrieve it:
...
def apply_theme(app: QApplication, theme_name: str) -> ThemeSpec:
    ...
    if app is not None:
        ...
        app._current_theme_spec = theme  # type: ignore[attr-defined]
    return theme
```
...
```bash
cd ui && git add src/fractal_studio/application/controllers/theme_controller.py src/fractal_studio/theme.py && git commit -m "feat: thread ViewportWell into ThemeController refresh"
```
...
In `main_window.py`, update `_finalize_startup` to capture sidebar state from `startup`:
...
Add `_on_section_collapsed` method to `MainWindow`:

```python
def _on_section_collapsed(self, section_key: str, collapsed: bool) -> None:
    current = self._settings_repo.load().settings
    from dataclasses import replace
    updated = replace(
        current,
        sidebar_collapsed={**current.sidebar_collapsed, section_key: collapsed},
    )
    self._settings_repo.save(updated)
```

And in `_init_window_state`, initialise the field:
...
def _init_window_state(self) -> None:
    ...
    self._startup_sidebar_collapsed: dict[str, bool] = {}  # NEW
```
