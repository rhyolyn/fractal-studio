...
class EditorController:
    ...
    def set_control_points(self, editor: ColorCubeEditor, control_points: list[Color]) -> None:
    ...
    def refresh_palette(self, editor: ColorCubeEditor) -> None:
        editor._palette = (
            self._backend.generate_palette(editor._control_points, self._profile.palette_size)
    ...
    def handle_mouse_press(self, editor: ColorCubeEditor, x: float, y: float) -> None:
        ...
        if point_index is None:
            ...
            self.refresh_palette(editor)
            editor.status_changed.emit(f"Added control point {len(editor._control_points) - 1} on face {face}.")
            editor.update()
            return

        editor._drag_state = DragState(face=face, point_index=point_index)
        editor.status_changed.emit(f"Dragging control point {point_index} on face {face}.")
    ...
    def handle_mouse_move(self, editor: ColorCubeEditor, x: float, y: float) -> None:
        ...
        editor._control_points[index] = self._backend.update_control_point_from_face(face, current, position)
        self.refresh_palette(editor)
        editor.update()
    ...
    def paint(self, editor: ColorCubeEditor, painter: QPainter) -> None:
        ...
        for face, rect in self.face_rects(editor).items():
            painter.drawPixmap(rect.toRect(), self.face_pixmap(editor, face, rect.size().toSize()))
        ...
        if not self._backend.available:
            painter.setPen(editor.palette().text().color())
            painter.drawText(editor.rect(), Qt.AlignmentFlag.AlignCenter, "Build the Rust extension to enable editing.")
    ...
    def face_at(self, editor: ColorCubeEditor, point: QPointF) -> tuple[int | None, tuple[float, float] | None]:
        for face, rect in self.face_rects(editor).items():
    ...
    def nearest_point(self, editor: ColorCubeEditor, face: int, point: QPointF) -> int | None:
        candidates = []
    ...
    def projected_point(self, editor: ColorCubeEditor, face: int, color: Color) -> QPointF:
    ...
    def draw_points(self, editor: ColorCubeEditor, painter: QPainter, face: int) -> None:
    ...
    def draw_spline(self, editor: ColorCubeEditor, painter: QPainter, face: int) -> None:
        projected = [self.projected_point(editor, face, color) for color in editor._control_points]
    ...
    def catmull_rom_channel(self, p0: float, p1: float, p2: float, p3: float, t: float) -> float:
        return 0.5 * (
