...
class FavoriteHoverPresenter:
    ...
    def build_stats_html(self, row: QWidget, favorite: dict) -> str:
        ...
        rows = [
            ...
            make_row("Center", f"{favorite['center_x']:.6f}, {favorite['center_y']:.6f}"),
        ...
        if favorite["is_julia"]:
            rows.append(make_row("Julia c", f"{favorite['julia_real']:.4f}+{favorite['julia_imag']:.4f}i"))
        rows.append(make_row("Power", str(favorite["power"])))
        if favorite["formula"].lower() in ("phoenix",):
            rows.append(make_row("Phoenix", f"{favorite['phoenix_real']:.4f}+{favorite['phoenix_imag']:.4f}i"))
        if favorite["coloring_mode"].lower().startswith("orbit_trap"):
            rows.append(make_row("Trap pt", f"{favorite['trap_x']:.3f}, {favorite['trap_y']:.3f}"))
